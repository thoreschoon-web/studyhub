...
import { Clock, Check, X, ChevronLeft, ChevronRight, Flag, RotateCcw } from "lucide-react";
...
export function ExamSimulator({ pools }: { pools: Pool[] }) {
    ...
    return (
      <div style={{ "--accent": pool?.accent } as React.CSSProperties}>
        ...
        <div className="space-y-3">
          {questions.map((q, i) => {
            const ok = grade(q, answers[q.id]?.value ?? null);
            return (
